...
bool ParkingService::registerVehicle(const std::string& plate, VehicleType type, int hour,
                                   std::chrono::microseconds& executionTime) {
    auto start = std::chrono::high_resolution_clock::now();
    
    Vehicle* newVehicle = new Vehicle(plate, type, hour);
...
void ParkingService::displayAllVehicles(std::chrono::microseconds& executionTime) {
    ...
    if (count == 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            std::cout << "  Hora de entrada: " << vehicles[i]->getEntryHour() << "\n";
            std::cout << "------------------------\n";
        }
    }
